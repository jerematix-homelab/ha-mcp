...
func TestQueryDevices_Health_Remove(t *testing.T) {
	t.Parallel()
	...
	tests := []struct {
		...
		{
			name: "missing device_ids parameter",
			args: map[string]any{
				"mode":   "health",
				"action": "remove",
			},
			setupMock:    func(_ *UniversalMockClient) {},
			wantError:    false,
			wantContains: []string{"device_ids", "required"},
		},
		{
			...
			wantError:    false,
			wantContains: []string{"not found", "nonexistent"},
		},
		{
			name: "integration does not support device removal",
			args: map[string]any{
				"mode":       "health",
				"action":     "remove",
				"device_ids": []any{"device1"},
			},
			setupMock: func(m *UniversalMockClient) {
				m.GetDeviceRegistryFn = func(_ context.Context) ([]homeassistant.DeviceRegistryEntry, error) {
					return []homeassistant.DeviceRegistryEntry{
						{ID: "device1", Name: "Device 1", ConfigEntries: []string{"entry1"}},
					}, nil
				}
				m.GetConfigEntriesFn = func(_ context.Context, _ string) ([]homeassistant.ConfigEntryFull, error) {
					return []homeassistant.ConfigEntryFull{
			...
			wantError:    false,
			wantContains: []string{"does not support device removal", "test_integration"},
		},
		{
			name: "successful device removal",
			args: map[string]any{
				"mode":       "health",
				"action":     "remove",
				"device_ids": []any{"device1"},
			...
			setupMock: func(m *UniversalMockClient) {
				...
				m.RemoveDeviceConfigEntryFn = func(_ context.Context, _, _ string) error {
					return nil
				}
			},
			wantError:    false,
			wantContains: []string{"Successfully Removed", "device1"},
		},
		{
			name: "partial removal success",
			args: map[string]any{
				"mode":       "health",
				"action":     "remove",
				"device_ids": []any{"device1", "device2"},
			},
			setupMock: func(m *UniversalMockClient) {
				m.GetDeviceRegistryFn = func(_ context.Context) ([]homeassistant.DeviceRegistryEntry, error) {
					return []homeassistant.DeviceRegistryEntry{
						{ID: "device1", Name: "Device 1", ConfigEntries: []string{"entry1"}},
						{ID: "device2", Name: "Device 2", ConfigEntries: []string{"entry2"}},
					}, nil
				}
				m.GetConfigEntriesFn = func(_ context.Context, _ string) ([]homeassistant.ConfigEntryFull, error) {
					return []homeassistant.ConfigEntryFull{
						{EntryID: "entry1", Domain: "test1", SupportsRemoveDevice: true},
						{EntryID: "entry2", Domain: "test2", SupportsRemoveDevice: false},
					}, nil
				}
				m.RemoveDeviceConfigEntryFn = func(_ context.Context, _, _ string) error {
					return nil
			...
			wantError:    false,
			wantContains: []string{"Successfully Removed", "device1", "does not support", "device2"},
		},
		{
			name: "JSON format remove result",
			args: map[string]any{
				"mode":       "health",
				"action":     "remove",
				"device_ids": []any{"device1"},
			...
			wantError:    false,
			wantContains: []string{`"successes"`, `"device_id": "device1"`},
		},
	}
