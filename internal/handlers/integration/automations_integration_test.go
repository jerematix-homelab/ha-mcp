...
func (s *AutomationIntegrationTestSuite) TestAutomationLifecycle() {
	...
	// IMPORTANT: Alias must match ID because HA derives entity_id from alias (slugified)
	automationConfig := homeassistant.AutomationConfig{
		ID:          automationID,
		Alias:       automationID, // Use same value so entity ID will be automation.<automationID>
	...
	// Wait for automation to appear in ListAutomations
	automation, err := s.WaitForAutomation(automationID, 30*time.Second)
	s.Require().NoError(err, "Automation did not appear")
	s.Require().NotNil(automation.Config, "Automation config should not be nil")
	s.Equal(automationID, automation.Config.Alias, "Automation alias should match")

	// Now verify it's also available via GetState
	automationEntityID := BuildEntityID("automation", automationID)
	entity, err := s.Client().GetState(s.Context(), automationEntityID)
	s.Require().NoError(err)
	// Note: State may be empty string initially, not "on", so we don't assert the state
	_ = entity // Used later in toggle tests
	...
	err = s.Client().ToggleAutomation(s.Context(), automationEntityID, false)
	...
	err = s.Client().ToggleAutomation(s.Context(), automationEntityID, true)
	...
	err = s.WaitForEntityGone(automationEntityID, 30*time.Second)
...
func (s *AutomationIntegrationTestSuite) TestAutomationUpdate() {
	...
	s.Equal(automationID, friendlyName)

	// Update automation
	updatedAlias := automationID + "_updated"
	updatedConfig := homeassistant.AutomationConfig{
		ID:          automationID,
		Alias:       updatedAlias, // Change alias to test update
	...
	// Call automation.reload to make changes visible
	_, err = s.Client().CallService(s.Context(), "automation", "reload", nil)
	s.Require().NoError(err, "Failed to reload automations")

	// Wait for reload to propagate (2s recommended)
	time.Sleep(2 * time.Second)
	entity, err = s.Client().GetState(s.Context(), automationEntityID)
	s.Require().NoError(err)

	friendlyName, _ = entity.Attributes["friendly_name"].(string)
	s.Equal(updatedAlias, friendlyName, "Automation name should be updated")
...
func (s *AutomationIntegrationTestSuite) TestAutomationWithCondition() {
	...
	_, _ = s.WaitForEntity(targetEntityID, 30*time.Second)
	_, _ = s.WaitForEntity(conditionEntityID, 30*time.Second)
	_, _ = s.WaitForEntity(triggerEntityID, 30*time.Second)
